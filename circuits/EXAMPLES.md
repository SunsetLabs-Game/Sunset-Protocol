# Sunset Circuits - Usage Examples

This document provides concrete examples of how to use Sunset's circuits for private operations.
...
async function proveMembership() {
    ...
    // Compute commitment (must match the Solidity verifier flow)
...
### Converting Proofs for Solidity Verifiers

After generating a proof, convert it to the format expected by the Solidity verifier stack:
...
```solidity
// Submit proof to Sunset contract
...
// Verify and execute
sunsetContract.executePrivateSwap(proof, publicInputs, newCommitment);
```
...
- Sunset repository documentation
