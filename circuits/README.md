# Sunset Circom Circuits

Production-ready zero-knowledge circuits for Sunset's shielded CLMM protocol on Conflux eSpace.

## Overview

This directory contains Circom circuits that prove private operations in Sunset without revealing sensitive information like balances, ownership, or transaction amounts. These circuits are **critical security components** - any vulnerability here can compromise the entire protocol.
...
**CRITICAL**: The Circom commitment scheme MUST match the Solidity contract commitment format exactly.
...
This matches the commitment structure expected by the Sunset Solidity verifier and coordinator contracts.
...
- ✅ **Poseidon hashing** - efficient for Circom proving and EVM-compatible verification flows
...
## Integration with Solidity on Conflux

### Commitment Compatibility

The Circom circuits generate commitments that **must** match the Solidity contracts:

**Solidity-side expectation**:
```solidity
...
- **Root**: Stored on-chain in the Sunset coordinator contract

### Verification Flow

1. **Off-chain**: User generates proof using Circom circuits
2. **On-chain**: Solidity contract verifies proof on Conflux eSpace
