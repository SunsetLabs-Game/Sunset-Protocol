...
import type { SunsetClient, Note, PositionNote } from "@sunset/sdk";
import { env } from "@/config/env";

const NOTES_STORAGE_KEY = "sunset_notes";
const PASSWORD_STORAGE_KEY = "sunset_password";

interface SdkState {
  client: SunsetClient | null;
...
export const useSdkStore = create<SdkState>((set, get) => ({
  ...
  initialize: async (password: string) => {
    ...
    try {
      const { SunsetClient } = await import("@sunset/sdk");
      const client = new SunsetClient({
        rpcUrl: env.rpcUrl,
