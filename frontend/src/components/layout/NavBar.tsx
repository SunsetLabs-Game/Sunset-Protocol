...
export function NavBar() {
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 h-16 border-b border-border/70 bg-canvas/78 backdrop-blur-md">
            ...
            >
              {({ isActive }) => (
                <>
                  {link.label}
                  {isActive && (
                    <span className="absolute bottom-0 left-3 right-3 h-0.5 bg-text-display rounded-full" />
                  )}
                </>
              )}
            </NavLink>
