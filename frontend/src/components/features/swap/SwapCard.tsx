...
import { FEE_TIERS, estimateSwapOutputSafe } from "@sunset/sdk";
import type { Note, PoolKey } from "@sunset/sdk";
...
const STORAGE_KEY = "sunset_recent_swaps";
    ...
    <div className="space-y-6 w-full max-w-lg mx-auto">
        ...
        <div className="relative z-10 flex flex-col gap-6 bg-surface/82 p-6 sm:p-8 rounded-[23px] w-full h-full">
          <div className="flex items-center justify-between pb-4 border-b border-border/40">
            ...
            <div className="flex items-center gap-2">
              <span className={cn(
                ...
                usePublicSwap ? "bg-signal-warning" : "bg-text-display animate-pulse"
              )} />
          ...
          <div className="relative z-10 flex flex-col gap-2">
            ...
            <div className="flex justify-center -my-5 relative z-20">
              <button
                onClick={handleFlip}
                className="group flex flex-shrink-0 items-center justify-center w-12 h-12 rounded-full border border-border/45 bg-surface/80 backdrop-blur-md shadow-lg transition-all duration-300 hover:scale-110 hover:border-text-display/30 hover:bg-surface-elevated hover:shadow-[0_10px_24px_rgba(17,17,17,0.08)]"
              >
                <svg width="20" height="20" viewBox="0 0 16 16" fill="none" className="text-text-caption group-hover:text-text-display transition-colors duration-300">
                  <path
          ...
          {parsedAmountIn > 0n && poolState && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="flex items-center justify-between p-4 rounded-xl border border-border/45 bg-gradient-to-r from-surface-elevated/80 to-surface/35"
            >
          ...
          <div className="flex flex-col gap-6 pt-4 border-t border-border/40">
            {/* Privacy toggle */}
            <div className="flex items-center justify-between rounded-xl border border-border/45 bg-gradient-to-r from-surface-elevated/90 to-surface/45 p-5 transition-all duration-300 hover:border-text-display/20">
              <div className="flex flex-col gap-1">
                ...
                <span className="text-xs text-text-caption font-medium">
                  <BlurFadeText text={usePublicSwap ? "Visible on Conflux" : "100% Cryptographic Privacy"} isPublic={usePublicSwap} />
                </span>
              </div>
              <button
                onClick={() => setUsePublicSwap(!usePublicSwap)}
                className="px-5 py-2.5 rounded-lg text-xs font-bold tracking-widest uppercase transition-all bg-surface-elevated border border-border/50 hover:border-text-display/35 hover:text-text-display hover:shadow-[0_10px_18px_rgba(17,17,17,0.08)]"
              >
            ...
            <div>
              <Button
                variant="primary"
                className="w-full h-16 text-lg font-bold tracking-widest uppercase shadow-lg shadow-black/10"
...

      {
        recentSwaps.length > 0 && (
          <Card className="space-y-4 bg-surface/55 backdrop-blur-3xl border border-border/60 p-6 sm:p-8 rounded-[24px] shadow-2xl shadow-black/8 mt-8">
            <h3 className="text-sm font-semibold tracking-widest uppercase text-text-caption">Recent Transactions</h3>
            <div className="space-y-3">
              {recentSwaps.map((tx) => (
                <div
                  key={tx.txHash}
                  className="group flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-border/45 bg-gradient-to-br from-surface-elevated/90 to-surface/45 transition-all duration-300 hover:border-text-display/25"
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-3">
                      ...
                      {tx.isPrivate && (
                        <span className="px-2 py-0.5 rounded-full bg-surface-elevated border border-border/45 text-[10px] uppercase tracking-widest text-text-display font-bold">
                          Shielded
                        </span>
                      )}
                    </div>
                  ...
                  <a
                    ...
                    className="mt-3 sm:mt-0 text-xs font-semibold tracking-widest uppercase text-text-display hover:text-text-heading transition-colors flex items-center gap-1.5"
                  >
                    Explorer
