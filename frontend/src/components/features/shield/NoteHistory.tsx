...
const HISTORY_STORAGE_KEY = "sunset_note_history";
const MAX_HISTORY_ITEMS = 10;

/**
 * Displays transaction history for deposits (shields) and withdrawals (unshields).
 * Loads from localStorage and provides links to a transaction explorer.
 */
export function NoteHistory() {
  ...
  useEffect(() => {
    ...
    const depositHistory = localStorage.getItem("sunset_recent_deposits");
    const withdrawHistory = localStorage.getItem("sunset_recent_withdrawals");
  ...
  if (history.length === 0) {
    return (
      <Card className="p-6 text-center bg-surface/45 backdrop-blur-xl border-border/50">
  ...
  return (
    <Card className="p-6 bg-surface/45 backdrop-blur-xl border-border/50">
        ...
        <AnimatePresence>
          {history.map((item, index) => (
            <motion.div
              ...
              className="flex items-center justify-between p-4 rounded-xl bg-surface-elevated/60 border border-border/45 hover:border-text-display/20 transition-all"
            >
                ...
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center ${
                    item.type === "deposit"
                      ...
                      : "bg-surface-elevated text-text-display"
                  }`}
                >
                  {item.type === "deposit" ? "↓" : "↑"}
                </div>
              ...
              <a
                ...
                className="flex items-center gap-1.5 text-xs font-bold tracking-widest uppercase text-text-display hover:text-text-heading transition-colors"
              >
                Explorer
