...
export function SettingsPage() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `sunset-backup-${Date.now()}.json`;
      a.click();
      URL.revokeObjectURL(url);
      localStorage.setItem("sunset_last_backup", Date.now().toString());
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      localStorage.setItem("sunset_notes", text);
        ...
        <div className="mt-8 space-y-4">
          ...
          <motion.div variants={itemVariants}>
            <div className="relative group overflow-hidden rounded-[24px] p-[1px] transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/10 w-full">
              ...
              <div className="relative z-10 bg-surface/82 backdrop-blur-xl p-6 sm:p-8 rounded-[23px] w-full h-full flex flex-col gap-4">
                <h2 className="text-xl font-bold tracking-tight text-text-display pb-4 border-b border-border/40">Export Backup</h2>
          ...
          <motion.div variants={itemVariants}>
            <div className="relative group overflow-hidden rounded-[24px] p-[1px] transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/10 w-full">
              ...
              <div className="relative z-10 bg-surface/82 backdrop-blur-xl p-6 sm:p-8 rounded-[23px] w-full h-full flex flex-col gap-4">
                <h2 className="text-xl font-bold tracking-tight text-text-display pb-4 border-b border-border/40">Import Backup</h2>
                <div className="rounded-xl border border-border/45 bg-gradient-to-r from-surface-elevated/90 to-surface/45 p-5">
                  <p className="text-sm text-text-caption leading-relaxed">
          ...
          <motion.div variants={itemVariants}>
            <div className="relative group overflow-hidden rounded-[24px] p-[1px] transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/10 w-full">
              ...
              <div className="relative z-10 bg-surface/82 backdrop-blur-xl p-6 sm:p-8 rounded-[23px] w-full h-full flex flex-col gap-4">
                <h2 className="text-xl font-bold tracking-tight text-text-display pb-4 border-b border-border/40">Lock Vault</h2>
                <div className="rounded-xl border border-border/45 bg-gradient-to-r from-surface-elevated/90 to-surface/45 p-5">
                  <p className="text-sm text-text-caption leading-relaxed">
          ...
          {/* Reset Vault */}
          <motion.div variants={itemVariants}>
            <Card className="space-y-4 bg-gradient-to-br from-[#8f6d49]/35 to-[#6c4b2f]/18 backdrop-blur-3xl border border-red-500/15 hover:border-red-500/30 p-6 sm:p-8 rounded-[24px] shadow-2xl shadow-black/8 transition-all duration-300">
