...
import { FEE_TIERS, getAmountsForBurn } from "@sunset/sdk";
import type { PositionNote, PoolKey } from "@sunset/sdk";
    ...
    <PageContainer>
      ...
      >
        ...
        {isInitialized && hasPositionsWithoutLeafIndex && (
          <motion.div variants={itemVariants} className="rounded-[24px] border border-border/60 bg-surface-elevated/65 p-6 backdrop-blur-xl shadow-lg shadow-black/5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex-1">
                <p className="text-sm font-bold tracking-widest uppercase text-text-display">Positions need sync</p>
        ...
        <div className="space-y-4">
          {!isInitialized ? (
            <motion.div variants={itemVariants}>
              <Card className="flex items-center justify-center p-8 bg-surface/55 backdrop-blur-3xl border border-border/60 rounded-[24px]">
            ...
            positions.map((pos) => (
              <motion.div variants={itemVariants} key={pos.commitment}>
                <div className="relative group overflow-hidden rounded-[24px] p-[1px] transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/10 w-full">
                  ...
                  <div className="relative z-10 bg-surface/80 backdrop-blur-xl p-6 sm:p-8 rounded-[23px] w-full h-full flex flex-col gap-6">
                    ...
                    <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div className="p-4 rounded-xl border border-border/45 bg-gradient-to-br from-surface-elevated/90 to-surface/45">
                    ...
                    {pos.txHash && (
                      <div className="mt-6 pt-4 border-t border-border/40 flex justify-end">
                        <a
                          ...
                          className="text-xs font-bold tracking-widest uppercase text-text-display hover:text-text-heading transition-colors inline-flex items-center gap-1.5"
                        >
                          View Transaction
      ...
      >
        <div className="space-y-4">
          ...
          {burnTarget && (
            <div className="rounded-xl border border-border/45 bg-gradient-to-r from-surface-elevated/90 to-surface/45 p-5 text-sm mt-4 space-y-3">
