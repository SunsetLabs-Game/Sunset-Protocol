/**
 * Main entry point for the Sunset SDK on Conflux eSpace.
 *
...
import { ConfluxRpcClient } from "./conflux/rpc.js";
import { PoolReader } from "./conflux/pool.js";
import { CoordinatorReader } from "./conflux/coordinator.js";
import { assertContractAddresses, type SunsetContractAddresses } from "./conflux/contracts.js";
import type { PoolKey, PoolState, Position, ProvingMode } from "./types/index.js";

export interface SunsetClientConfig {
  rpcUrl: string;
...
export class SunsetClient {
  private config: SunsetClientConfig;
  private provider: ConfluxRpcClient;
  ...
  constructor(config: SunsetClientConfig) {
    const contracts = assertContractAddresses(config.contracts);
    this.config = {
      ...config,
      contracts,
    };
    ...
    this.provider = new ConfluxRpcClient(config.rpcUrl);
    this.poolReader = new PoolReader(this.provider, contracts.pool);
